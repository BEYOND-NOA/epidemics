...
landsat8Collection = ee.ImageCollection([])  # the filtered collection of images.
...
def _landsat8_products_indices_creator(image):
    # normalized indices creation.
    ndbi = geetils_standard._l8_ndbi_calculator(image)
    ndmi = geetils_standard._l8_ndmi_calculator(image)
    ndvi = geetils_standard._l8_ndvi_calculator(image)
    ndwi = geetils_standard._l8_ndwi_calculator(image)
...
def _load_configuration(iniFilePath):
    ...
    configuration = {
        "wrs": config.items("Landsat8_WRS"),

        "startDate": config["Landsat8"]["startDate"],
        "endDate": config["Landsat8"]["endDate"],
        "monthlyStep": config["Landsat8"]["monthlyStep"],
        "crs": config["Landsat8"]["crs"],
        "scale": config["Landsat8"]["scale"],
        "bucket": config["Landsat8"]["bucket"],
        "fileFormat": config["Landsat8"]["fileFormat"],
        "maxPixels": config["Landsat8"]["maxPixels"],
        "aoi": config["Areas_EPSG4326"][config["Landsat8"]["area"]],
    }
...
def _image_export_task_scheduler(startDatesList, endDatesList, wrsDictionary, aoi, bucket, crs, scale, fileFormat, maxPixels):
    ...
    initialLandsat8Collection = geetils_standard._collection_creator("LANDSAT/LC08/C01/T1_SR", startPeriod, endPeriod, aoi)

    # further filtering the initial image collection.
    for key, value in wrsDictionary.items():
        tempLandsat8Collection = initialLandsat8Collection \
            .filter(ee.Filter.Or(
            ee.Filter.And(ee.Filter.eq("WRS_PATH", key), ee.Filter.inList("WRS_ROW", value))
        ))
        landsat8Collection = landsat8Collection.merge(tempLandsat8Collection)

    landsat8Collection = landsat8Collection \
    ...
    # informational message.
    print("Found", landsat8Collection.size().getInfo(), "images.")
    print("Attempting to create daily mosaics from the existing images.")

    acquisitionDates = ee.List(geetils_standard._date_extractor(landsat8Collection))
    ...
    mosaicCollection = geetils_standard._temporal_collection(landsat8Collection, ["description"], acquisitionDates, 1, "day")

    # create the indices, clip the images.
    mosaicCollection = mosaicCollection.map(lambda image: _landsat8_products_indices_creator(ee.Image(image)))
...
def main():
    ...
    print("This run will attempt to create an image collection from", datesList[0], "till", datesList[-1],
          "of USGS Landsat 8 SR Reflectance Tier 1 images.\n")
